#!/bin/bash -l
#SBATCH -p intel -N 1 -n 2 --mem 32gb --out logs/HiC_SALSA_fixmisasm.%a.log
...
if [ ! -s $INPUT/$BED ]; then
	echo 'no BED file, expected it to be here already'
	exit
...
$pipeline -a $GENOME -l $GENOME.fai -b $INPUT/$BED -e GATC -o $OUT/$NAME.correct --exp 29000000 -m yes
