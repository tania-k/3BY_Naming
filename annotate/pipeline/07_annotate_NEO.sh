#!/usr/bin/bash -l
#SBATCH --nodes=1
#SBATCH --ntasks=16 --mem 120gb
#SBATCH --output=logs/annotfunc.%a.log
#SBATCH -p short -C xeon -J annotfunc
...
do
  ...
  TEMPLATE=$(realpath lib/Blackyeast.sbt)
  if [ ! -f $TEMPLATE ]; then
  ...
  funannotate annotate --sbt $TEMPLATE --busco_db $BUSCO -i annotate/Neophaeococcomyces_aloes_FJII-L3-CM-P2 --species "aloes" --strain "FJII-L3-CM-P2" --cpus $CPUS $MOREFEATURE $EXTRAANNOT
done
